...
- **A new prototype's filename goes in `PAGES` in `tests.html`, in the same commit.** The suite cannot list the directory it is served from, so a page missing from that array is a page with no CSP check at all — which is the exact hole the suite exists to close. `README.md`'s file table says so too.
...
- New prototypes are designed against the Midnight palette (deep indigo/navy) — the family's base palette — but OPEN on `auto`, the family default since 2026-08-22, which follows the reader's system and resolves to Light or Midnight. Canonical source: the private `claude-theme-pack` repo — copy its generated `theme.css` or transcribe from its `tokens.json`. A page with no picker still carries `data-theme="auto"` on `<html>`: that attribute is the whole of its theme, and without it the page takes `:root`'s midnight whatever the reader's system says.
- After changes: **browser-test locally first**, then commit, push, verify the Pages deploy, and spot-check live. To serve locally, use whatever your environment provides: the desktop app's preview pane reads `.claude/launch.json` (port 8019); otherwise run `python3 -m http.server 8019` in this folder and drive a browser with whatever automation is available (e.g. Playwright). Don't spend time hunting for a specific tool — any local server + browser works. These prototypes are candidates for eventual native iOS/macOS versions once polished.

- Write commit subject lines in plain English a non-developer can read (what changed and why it matters, not implementation detail).
